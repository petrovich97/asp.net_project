...
namespace EHotelServices.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                Gost user = dbc.Gosti.Where(x => x.Username == username).FirstOrDefault();
            ...
            using (ApplicationDbContext dbc = new ApplicationDbContext())
            {
                Osoblje user = dbc.Zaposleni.Where(x => x.Username == username).FirstOrDefault();
                if (user != null && user.Odobren==false)
                {
                    ModelState.AddModelError("", "Account is not allowed.");
                    return View(model);
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    return RedirectAccType(username);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (model.Type == "Guest")
                        return RedirectToAction("Create", "Gosts", new { email = model.Email });
                    if (model.Type == "Crew")
                        return RedirectToAction("Create", "Osobljes",new { email = model.Email });
                }
                AddErrors(result);
            }
        ...
        private ActionResult RedirectAccType(string username)
        {
            using (ApplicationDbContext dbc = new ApplicationDbContext())
            {
                ...
                Administrator a = dbc.Admin.Where(x => x.Username == username).FirstOrDefault();
